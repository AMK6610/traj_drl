...
def create_data_loaders_simple(weak_transformation, strong_transformation,
    ...
    dataset = db_semisuper.DBSS(traindir, labels, False, args.aug_num, eval_transformation, weak_transformation,
                                strong_transformation)

    sampler = SubsetRandomSampler(dataset.labeled_idx)
    batch_sampler = BatchSampler(sampler, args.batch_size, drop_last=True)
    train_loader = torch.utils.data.DataLoader(dataset, batch_sampler=batch_sampler, num_workers=args.workers,
                                               pin_memory=False)
    ...
    batch_sampler_u = BatchSampler(SubsetRandomSampler(dataset.unlabeled_idx),
                                   batch_size=args.batch_size - args.labeled_batch_size, drop_last=True)
...
def create_model(num_classes, args):
    ...
    if model_choice == "resnet18":
        ...
        model = build_wideresnet(28, 2, 0, num_classes, 2 * 64)

    elif model_choice == "wrn-28-8":
        model = build_wideresnet(28, 8, 0, num_classes, 8 * 64)
...
def train_sup(train_loader, model, optimizer, epoch, global_step, args, ema_model=None):
    ...
    for i, (aug_images, target) in enumerate(train_loader):
        ...
        for batch in aug_images:
            ...
            class_logit, _, _ = model(m_batch, use_param_t=True)
            if count == 0:
...
def validate(eval_loader, model, args, global_step, epoch, num_classes=10):
    ...
    with torch.no_grad():
        for batch_idx, (inputs, targets) in enumerate(eval_loader):
            ...
            outputs, _, _ = model(inputs, use_param_t=True)
        ...
        logging.info(' * Prec@1 {top1.avg:.3f}\tPrec@5 {top5.avg:.3f}'.format(top1=meters['top1'], top5=meters['top5']))
...
def extract_features_simp(train_loader, model, args):
    ...
    with torch.no_grad():
        for i, (batch_input) in enumerate(train_loader):
            ...
            _, feats, _ = model(X_n, use_param_t=True)
...
def load_args(args):
    args.workers = 1  # 4 * torch.cuda.device_count()
    label_dir = '../LaplaceNet/data-local/'
